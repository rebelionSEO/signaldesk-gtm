...
const taskId=crypto.randomUUID();const today=new Date().toISOString().slice(0,10);const prepared=await call('/api/projects/'+id+'/operations','POST',{action:'prepare',revision:2,id:taskId,experimentId:'O1',owner:'Developer marketing',startDate:today,dueDate:today,budgetCap:500});assert.equal(prepared.status,200,JSON.stringify(prepared.data));assert.equal(prepared.data.operations.tasks.length,1);assert.match(prepared.data.operations.tasks[0].artifact,/Nothing published, sent, booked or spent/);
