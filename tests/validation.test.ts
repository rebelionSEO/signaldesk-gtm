...
test('V3 export includes thesis, assumption map, benchmark cohort, and creative territory', () => { const md = toMarkdown(example); assert.ok(md.includes('## Market context')); assert.ok(md.includes('## Assumption map')); assert.ok(md.includes('## Benchmark cohort')); assert.ok(md.includes('Range unavailable') || md.includes('No defensible range')); assert.ok(md.includes('Territory: Product as proof')); });
