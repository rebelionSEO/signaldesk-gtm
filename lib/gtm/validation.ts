...
export function referenceErrors(report: z.infer<typeof contentSchema>, allowedUrls?: string[]) {
    ...
    const modeled=new Set<string>(); for(const m of report.economics??[]){if(!opportunityIds.has(m.experimentId))errors.push('Economics references missing experiment '+m.experimentId);if(modeled.has(m.experimentId))errors.push('Duplicate economics model '+m.experimentId);modeled.add(m.experimentId);}
