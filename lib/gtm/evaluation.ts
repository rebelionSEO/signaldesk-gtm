...
const genericPattern = /\b(webinar|podcast|ebook|report|benchmark|index|playbook|landing page|case stud(?:y|ies)|thought leadership|email sequence|retargeting|community|linkedin posts?|seo content)\b/i;
...
export function evaluateReport(report: Report, repeatedReports: Report[] = []): EvaluationResult {
  ...
  const creativeContract = designs.length > 0 && designs.every(item => complete(item.design?.categoryConvention) && complete(item.design?.brandFit) && complete(item.design?.commercialRole) && complete(item.design?.assumptionToTest) && complete(item.design?.proofRequired));
  const brandProfile = report.brandProfile;
  const brandEvidence = Boolean(brandProfile?.voiceTraits.length && brandProfile.voiceTraits.every(item => item.evidenceIds.length > 0 && item.evidenceIds.every(id => evidenceIds.has(id))) && brandProfile.signatureAssets.every(item => item.evidenceIds.length > 0 && item.evidenceIds.every(id => evidenceIds.has(id))));
  const commercialRoles = unique(designs.map(item => item.design?.commercialRole ?? ''));
  const genericOnly = designs.filter(item => genericPattern.test(`${item.title} ${item.action}`) && (!complete(item.design?.differentBecause) || !complete(item.design?.brandFit) || !complete(item.design?.categoryConvention) || !complete(item.design?.assumptionToTest))).length;
  const originalityScore = cap(Math.min(ambition.size, 3) * 6 + Math.min(territories, Math.max(report.opportunities.length, 1)) / Math.max(report.opportunities.length, 1) * 16 + Math.min(distinctReasons, Math.max(designs.length, 1)) / Math.max(designs.length, 1) * 12 + Math.min(commercialRoles, 3) * 5 + (brandEvidence ? 14 : 0) + (creativeContract ? 15 : 0) + (designs.length > 0 && designs.every(item => complete(item.design?.mechanism)) ? 10 : 0) + (designs.length > 0 && designs.every(item => complete(item.design?.smallestTest) && complete(item.design?.stopRule)) ? 10 : 0) - genericOnly * 15);
  ...
  const dimensions: EvaluationDimension[] = [
    ...
    { id: 'originality', label: 'Brand-native originality', score: originalityScore, weight: 20, status: statusFor(originalityScore), explanation: 'Checks sourced brand permission, category cliches, distinct commercial jobs, company-specific mechanisms, assumptions, and cheap disproofs.', nextAction: originalityScore >= 80 ? 'Test whether the brand-native mechanism changes qualified commercial behavior.' : 'Replace logo-swappable formats with a mechanism built from evidenced company assets and a clear commercial job.' },
  ...
  const gates: EvaluationGate[] = [
    ...
    { name: 'Brand-native creativity', pass: brandEvidence && creativeContract && commercialRoles >= Math.min(3, report.opportunities.length), detail: 'The plan needs an evidence-linked creative brief and every idea must replace a category convention, fit the brand, perform a commercial job, and state its disproof.' },
