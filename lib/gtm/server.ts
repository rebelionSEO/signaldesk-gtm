...
export function publicStudy(row: StudyRow) { return { id: row.id, report: JSON.parse(row.report), stage: row.stage, revision: row.revision, busy: row.busy_until > Date.now(), updatedAt: row.updated_at }; }
