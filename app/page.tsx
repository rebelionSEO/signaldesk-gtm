...
import { ChannelBoard, DecisionBrief, ExecutionBoard, ExperimentDesign, SystemBoard } from '@/components/gtm-v2';
type Study = {
    id: string;
    report: Report;
...
export default function Home() {
    ...
    return <main className="workspace"><header className="topbar"><Link className="brand" href="/"><Radio size={23}/>signaldesk<span> / GTM workspace</span></Link><span className="private-label"><ShieldCheck size={15}/> Evidence before action</span></header><div className="shell">
 ...
 <Tabs value={tab} onValueChange={setTab}><TabsList className="tabbar" variant="line"><TabsTrigger value="overview">Overview</TabsTrigger><TabsTrigger value="channels">Channels</TabsTrigger><TabsTrigger value="plan">Experiments <span>{report.opportunities.length}</span></TabsTrigger><TabsTrigger value="execution">Execution <span>{study?.operations.tasks.length ?? 0}</span></TabsTrigger><TabsTrigger value="evidence">Evidence <span>{report.evidence.length}</span></TabsTrigger><TabsTrigger value="system">System</TabsTrigger><TabsTrigger value="brief">Brief</TabsTrigger></TabsList>
 <TabsContent value="overview"><DecisionBrief report={report}/><div className="overview-grid"><section className="thesis"><p className="eyebrow">WORKING STRATEGY</p><h2>{report.summary}</h2><p>{report.brief.objective}</p><div className="thesis-foot"><span>Hypothesis · Needs validation</span><FlaskConical size={24}/></div></section><section className="panel snapshot"><p className="eyebrow">RESEARCH AT A GLANCE</p><div className="metrics"><div><strong>{sourceCount}</strong><span>Unique source pages</span></div><div><strong>{report.opportunities.length}</strong><span>Proposed experiments</span></div></div><hr /><p><ShieldCheck size={18}/>{positives ? 'Counterevidence included' : 'Counterevidence still needed'}</p><small>Evidence records can share a source. No measured business impact.</small></section></div>
 ...
 <TabsContent value="channels"><div className="section-title"><div><h2>One GTM, seven channel decisions</h2><p className="muted">The system prioritizes where to test, what to investigate and what to hold. It does not force activity into every channel.</p></div></div><ChannelBoard report={report}/></TabsContent>
 <TabsContent value="execution"><ExecutionBoard report={report} operations={study?.operations ?? emptyOperations} canPrepare={!!editable} onPrepare={opportunity => void prepare(opportunity)}/></TabsContent>
 <TabsContent value="system"><SystemBoard planner={study?.planner ?? null} traces={study?.trace ?? []} report={report}/></TabsContent>
